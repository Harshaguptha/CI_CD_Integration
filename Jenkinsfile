...
node('node'){
   stage('git checkout'){
      try {
       git credentialsId: 'git-token', url: 'https://github.com/Harshaguptha/CI_CD_Integration.git'
   ...
   stage('artifacts to s3'){
      try {
         withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'deploytos3', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
            sh "aws s3 ls"
            sh "aws s3 mb s3://addressbook-bucket-for-aws"
            sh "aws s3 cp addressbook_main/target/addressbook.war s3://addressbook-bucket-for-aws"
         }
      } catch(err) {
